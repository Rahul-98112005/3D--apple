  ...
  useEffect(() => {
    if (loadedData.length > 3) {
      ...
      if (currentVideo) {
        ...
        console.error(`Video element for ID ${videoId} is not available.`);
      }
    }
  ...
  useEffect(() => {
    ...
    if (span[videoId]) {
      ...
      const animUpdate = () => {
        ...
        if (!currentVideo) {
          console.error(`Video with ID ${videoId} is not available.`);
          return;
        }

        anim.progress(currentVideo / hightlightsSlides[videoId].videoDuration);
      };
      if (isPlaying) {
    ...
    <>
      <div className="flex items-center">
        {hightlightsSlides.map((list, i) => (
          <div key={list.id} id="slider" className="sm:pr-19 pr-10">
            <div className="video-carousel_container ">
              ...
              <div className="absolute top-12 left-[5%] z-10">
                {list.textLists.map((text) => (
                  <p
                    key={text}
                    className="md:text-2xl text-xl font-medium mx-4"
                  >
                    {text}
                  </p>
                ))}
      ...
      <div className="relative flex-center mt-10">
        <div className="flex-center py-5 px-7 bg-gray-300 backround-blur rounded-full ">
